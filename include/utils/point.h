...
template <typename T> BasePoint<T>::BasePoint(T y, T x) : y(y), x(x) {}

template <typename T> BasePoint<T>::BasePoint(const BasePoint &other) {
  if (this != &other) {
    x = other.x;
    y = other.y;
  }
...
template <typename T> BasePoint<T> &operator++(BasePoint<T> &point) {
  ++point.x;
  ++point.y;
  return point;
}

template <typename T> BasePoint<T> operator++(BasePoint<T> &point, int) {
